const API_BASE = "http://10.0.2.2:8080/api"; // backend Spring Boot
...
export async function searchNotes(keyword: string) {
  ...
  const res = await fetch(
    `${API_BASE}/notes/search?keyword=${encodeURIComponent(keyword)}`,
    {
      headers: getAuthHeaders(),
    }
  );
...
export async function updateNoteStatus(id: number, isCompleted: boolean) {
  ...
  const res = await fetch(
    `${API_BASE}/notes/${id}/status?isCompleted=${isCompleted}`,
    {
      method: "PUT",
      headers: getAuthHeaders(),
    }
